...
Besides the inherent statistical assumptions and speed performance, the choice of tuning grid ranges depends largely on mean-centering and unit-variance scaling all columns in `X` and `Y` - mean-centering in currently implemented in `gflasso` and we do advise to consider scaling `X` and `Y` beforehand. 
