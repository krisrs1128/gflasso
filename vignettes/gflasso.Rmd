...
Besides the inherent statistical assumptions and speed performance, the choice of tuning grid ranges depends largely on mean-centering and unit-variance scaling all columns in `X` and `Y`, which is currently automated. 
...
```{r}
system.time(CV <- cv_gflasso(X = SNP, Y = BM, R = DS, nCores = 2,
...
```{r}
gfMod <- gflasso(X = SNP, Y = BM, R = DS, opts = list(lambda = CV$optimal$lambda,
